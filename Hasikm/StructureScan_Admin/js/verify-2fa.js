  ...
  .addEventListener("submit", function (e) {
    ...
    db.collection("verificationCodes")
      ...
      .then((doc) => {
        ...
        // ✅ Code is valid! Delete verification code and proceed
        db.collection("verificationCodes")
          .doc(adminUid)
          .delete()
          .then(() => {
            ...
            // Store verified status in localStorage for 24 hours
            const nowMs = Date.now();
            const expiresIn24h = nowMs + 24 * 60 * 60 * 1000; // 24 hours

            localStorage.setItem("2fa_verified", "true");
            localStorage.setItem("2fa_verified_expires", String(expiresIn24h));
          ...
          .catch((error) => {
            ...
            // Even if delete fails, still allow login (code is verified)

            const nowMs = Date.now();
            const expiresIn24h = nowMs + 24 * 60 * 60 * 1000;
