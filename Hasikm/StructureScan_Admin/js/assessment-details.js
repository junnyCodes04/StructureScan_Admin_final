...
// ✅ FIXED: Load Detection Summary by counting actual detectedIssues
function loadDetectionSummary(data) {
  ...
  // Count issues from assessments array
  if (data.assessments && Array.isArray(data.assessments)) {
    data.assessments.forEach((assessmentWrapper) => {
      // Access nested structure
      const assessment = assessmentWrapper[0] || assessmentWrapper;
      const detectedIssues = assessment.detectedIssues || [];

      // Handle if detectedIssues is also wrapped in numeric index
      let issues = detectedIssues;
      if (
        detectedIssues[0] &&
        typeof detectedIssues[0] === "object" &&
        detectedIssues[0][0]
      ) {
        issues = detectedIssues.map((issue) => issue[0]);
      }
      ...
      issues.forEach((issue) => {
        ...
        // Count by severity
        const levelLower = (issue.level || "").toLowerCase();
  ...
  // ✅ Animate severity bars with percentages displayed inside
...
// ✅ FIXED: Load recommendations with correct nested structure
function loadRecommendations(data) {
  ...
  try {
    ...
    data.assessments.forEach((assessmentWrapper, assessmentIndex) => {
      console.log(
        `📋 Processing assessment ${assessmentIndex}:`,
        assessmentWrapper
      );

      // ✅ FIX: Access the nested object at index 0
      const assessment = assessmentWrapper[0] || assessmentWrapper;
      ...
      recommendations.forEach((recommendationWrapper, recIndex) => {
        console.log(
        ...
        if (recommendationWrapper[0]) {
          actionsMap = recommendationWrapper[0];
        } else if (recommendationWrapper["0"]) {
          actionsMap = recommendationWrapper["0"];
        } else {
          actionsMap = recommendationWrapper;
        }
...
// ✅ Store assessment data globally for modal access
let assessmentsData = [];

// ✅ FIXED: Load submitted images with correct structure
async function loadSubmittedImages(data, assessmentDate) {
  ...
  try {
    ...
    // Store assessments globally for modal access
    assessmentsData = data.assessments;

    imagesGrid.innerHTML = "";

    data.assessments.forEach((assessmentWrapper, index) => {
      // ✅ FIX: Access the nested object
      const assessment = assessmentWrapper[0] || assessmentWrapper;
      ...
      const imageUri = assessment.imageUri || assessmentWrapper.imageUri || "";
...
// ✅ FIXED: Modal with correct nested structure access
function openImageModal(index) {
  console.log("🖼️ Opening modal for image", index + 1);

  // Get assessment data from global array
  const assessmentWrapper = assessmentsData[index];
  ...
  // Set image - try both possible locations
  const imageUri = assessment.imageUri || assessmentWrapper.imageUri || "";
  ...
  // ✅ FIX: Access detectedIssues from the correct nested structure
  let detectedIssues = assessment.detectedIssues || [];
  ...
  if (
    ...
    detectedIssues = detectedIssues.map((issue) => issue[0]);
  }
